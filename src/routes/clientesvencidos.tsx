...
export const Route = createFileRoute("/clientesvencidos")({
  head: () => ({
    meta: [{ title: "Clientes vencidos" }],
  }),
  ...
  component: ClientesVencidosPage,
});
...
function ClientesVencidosPage() {
  ...
  const clientes = allStatuses.filter((c) => c.estado === "Vencido");

  return (
    <AppShell title="Clientes vencidos">
      <div className="space-y-2">
        <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">Clientes vencidos</h1>
        <p className="text-sm text-muted-foreground">
          {clientes.length} clientes con declaraciones vencidas
        </p>
      </div>
